...
par_ffpool = data[runs+2*par_rows:runs+3*par_rows, :]
par_ff = data[runs+3*par_rows:runs+4*par_rows, :]
...
for i in range(0, par_rows, runs):
    ...
    par_ffpool_times.append(np.mean(par_ffpool[i:i+runs, :], axis=0)[1])
...
scal = par_ffpool_times[0] / par_ffpool_times
